...
if SFS:
    ...
    for i in range(num_folds):
        ...
        # Train a LightGBM model for the current fold
        lgb_model = lgb.LGBMRegressor(**lgb_params)
        sfs = SequentialFeatureSelector(lgb_model, n_features_to_select=0.9)
        sfs.fit(df_fold_train[feature_columns],df_fold_train_target)
        # lgb_model.fit(
        #     df_fold_train[feature_columns],
        #     df_fold_train_target,
        #     eval_set=[(df_fold_valid[feature_columns], df_fold_valid_target)],
        ...
        #     ],
        # )
        sfs_result = sfs.get_support()
        for index, value in enumerate(sfs_result):
            logger.info(f"feats {index} support {value}")

        sfs_results.append(sfs_result.tolist())
